...
def inrn_v2(lin):
    ...
    l1 = bn(conv(lin, n_base_filter, filter_size=1))

    l2 = bn(conv(lin, n_base_filter, filter_size=1))
    l2 = bn(conv(l2, n_base_filter, filter_size=3))

    l3 = bn(conv(lin, n_base_filter, filter_size=1))
    l3 = bn(conv(l3, n_base_filter, filter_size=3))
    l3 = bn(conv(l3, n_base_filter, filter_size=3))

    l = lasagne.layers.ConcatLayer([l1, l2, l3])

    l = bn(conv(l, lin.output_shape[1], filter_size=1))
...
def inrn_v2_red(lin):
    ...
    l2 = bn(conv(lin, ins // den * nom2, filter_size=3, stride=2))

    l3 = bn(conv(lin, ins // den * nom2, filter_size=1))
    l3 = bn(conv(l3, ins // den * nom3, filter_size=3, stride=2))

    l4 = bn(conv(lin, ins // den * nom2, filter_size=1))
    l4 = bn(conv(l4, ins // den * nom3, filter_size=3))
    l4 = bn(conv(l4, ins // den * nom4, filter_size=3, stride=2))
