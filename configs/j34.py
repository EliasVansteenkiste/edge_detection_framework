...
def build_objective(model, deterministic=False, epsilon=1.e-7):
    ...
    Beta = 0.1
    entropy = - preds * T.log(preds) - (1-preds)*T.log(1-preds)
    Gamma = 0.1
    th_confidence_penalty = - Beta * T.clip(Gamma-entropy, 0., 1e7)
    th_confidence_penalty = T.mean(th_confidence_penalty)

    l2reg = nn.regularization.l2(predictions)                                                                                                                                                                                       
    weight_decay=0.00004

    return T.mean(weighted_bce) + th_confidence_penalty + weight_decay * l2reg 
