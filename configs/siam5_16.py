...
def build_model():
    ...
    l_fh = nn.layers.SliceLayer(l, indices=slice(0,l.output_shape[1]/2), axis=1)
    l_sh = nn.layers.SliceLayer(l, indices=slice(l.output_shape[1]/2,l.output_shape[1]),axis=1)

    l_fh = nn.layers.GlobalPoolLayer(l_fh, pool_function=T.mean)
    l_sh = nn.layers.GlobalPoolLayer(l_sh, pool_function=T.max)

    l_neck = nn.layers.ConcatLayer([l_fh, l_sh])
