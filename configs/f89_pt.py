...
learning_rate_schedule = {
    0: 1e-2,
    int(max_nchunks * 0.3): 3e-3,
    int(max_nchunks * 0.6): 1e-3,
    int(max_nchunks * 0.8): 3e-4,
}
...
def build_updates(model, learning_rate):
    ...
    ignored_params = list(map(id, model.densenet.classifier.parameters()))
    base_params = filter(lambda p: id(p) not in ignored_params,
                         model.densenet.parameters())

    optimizer = optim.SGD([
        {'params': base_params},
        {'params': model.densenet.classifier.parameters(), 'lr': learning_rate}
    ], lr=learning_rate * 0.1, momentum=0.9,weight_decay=0.0002)
