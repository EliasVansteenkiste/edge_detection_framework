...
# transformations
p_transform = {'patch_size': (299, 299),
               'channels': 3,
               'n_labels': 17}
...
def my_inception_v3(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model = torchvision.models.inception.Inception3(**kwargs)
        model.load_state_dict(torch.utils.model_zoo.load_url(torchvision.models.inception.model_urls['inception_v3_google']))
        return model

    return torchvision.models.inception.Inception3(**kwargs)

class Net(nn.Module):
    def __init__(self):
        super(Net, self).__init__()
        self.inception = my_inception_v3(pretrained=True)
        self.inception.fc = nn.Linear(self.inception.fc.in_features, p_transform["n_labels"])
        self.inception.fc.weight.data.zero_()
