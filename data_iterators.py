...
class LunaLungMaskDataGenerator():
    def __init__(self, data_path, transform_params, data_prep_fun, patient_ids, **kwargs):
    ...
    def generate(self):
        for patient_path in self.patient_paths:
            ...
            lung_mask = self.data_prep_fun(data=img,
                                             pixel_spacing=pixel_spacing,
                                             luna_origin=origin)
...
class DSBLungMaskDataGenerator(object):
    def __init__(self, data_path, transform_params, data_prep_fun, patient_ids, part_out_of=(1, 1)):

        self.patient_paths = utils_lung.get_patient_data_paths(data_path)

        if patient_ids is not None:
            self.patient_paths = [data_path + '/' + p for p in patient_ids]
    ...
    def generate(self):
        for p in self.patient_paths:
            ...
            lung_mask = self.data_prep_fun(data=img, pixel_spacing=pixel_spacing)
