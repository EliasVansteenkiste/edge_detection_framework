...
# transformations
p_transform_train = {'vector_size': 6784+3825+1024,
               'channels': 0,
               'n_labels': 17,
               'num_aug': 8}

p_transform_valid = {'vector_size': 6784+3825+1024,
               'channels': 0,
               'n_labels': 17,
               'num_aug': 1}
p_transform_test = {'vector_size': 6784+3825+1024,
               'channels': 0,
               'n_labels': 17,
               'num_aug': 8}
...
config_names = [
    ...
    "e23_f10-9-20170719-104857-best",
    "f113-f10_9_pt-20170719-052623-best",
]
...
tta_test_data_iterator = data_iterators.EnsembleTTADataGenerator(dataset=config_names,
                                                    duplicate_label = False,
                                                    img_ids = test_ids,
                                                    p_transform=p_transform_test,
                                                    data_prep_fun = data_prep_function_valid,
                                                    label_prep_fun = label_prep_function,
                                                    rng=rng,
                                                    full_batch=False, random=False, infinite=False,
...
tta_test2_data_iterator = data_iterators.EnsembleTTADataGenerator(dataset=config_names,
                                                    duplicate_label = False,
                                                    img_ids = test2_ids,
                                                    p_transform=p_transform_test,
                                                    data_prep_fun = data_prep_function_valid,
...
tta_valid_data_iterator = data_iterators.EnsembleTTADataGenerator(dataset=config_names,
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.batchnorm = nn.BatchNorm1d(p_transform_train["vector_size"])
        self.fc1 = nn.Linear(p_transform_train["vector_size"], 1024)
        self.classifier_drop = nn.Dropout(p=0.5)
        self.fc2 = nn.Linear(1024, p_transform_train["n_labels"])
...
def build_updates(model, learning_rate):
    return optim.SGD(model.parameters(), lr=learning_rate,momentum=0.9,weight_decay=0.0033)
