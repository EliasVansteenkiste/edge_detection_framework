...
def my_densenet161(pretrained=False, **kwargs):
    r"""Densenet-121 model from
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MyDenseNet(num_init_features=96, growth_rate=48, block_config=(6, 12, 36, 24))
    if pretrained:
        model.load_state_dict(torch.utils.model_zoo.load_url(torchvision.models.densenet.model_urls['densenet161']))
    return model
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.densenet = my_densenet161(pretrained=True)
