...
# losses
class MAE_wSE(torch.nn.modules.loss._Loss):

    def __init__(self, se_weight=9.):
        super(MAE_wSE, self).__init__()
    ...
    def forward(self, pred, target):
        ...
        #Absolute error symmetric component (noise robust)
        ae = target * (1-pred)  + (1-target) * pred

        #Weighted Square error for smoothness close to zero and for weighing  
        wse = self.se_weight * target * (1-pred)**2  + (1-target) * pred**2 

        #divide by two to have about the same numbers as ae or wse seperately
        loss = (ae + wse)/2
