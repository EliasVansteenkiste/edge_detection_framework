...
def my_densenet121(pretrained=False, **kwargs):
    ...
    """
    model = MyDenseNet(num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16),num_classes=17)
...
def build_model():
    ...
    metadata_path = utils.find_model_metadata(metadata_dir, config_name, best=True)
    metadata = utils.load_pkl(metadata_path)
    net.load_state_dict(metadata['param_values'])

    net.densenet.classifier = nn.Linear(net.densenet.classifier.in_features, p_transform["n_labels"])
...
def score(gts, preds):
    return fbeta_score(gts,preds>0.5,2)
