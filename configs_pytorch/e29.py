...
class MultiLoss(torch.nn.modules.loss._Loss):
    ...
    def forward(self, input, target):
        ...
        abs_errors = torch.abs(input-target)
        abs_err_gt = abs_errors.gt(0.5)
        abs_err_gt = abs_err_gt.float()

        weighted_sqerr = (self.weight*target)*(input-target)**2 +(1-target)*(input-target)**2
        weighted_err = (self.weight*target)*(torch.abs(input-target)-0.25) +(1-target)*(torch.abs(input-target)-0.25)
        #gradient is continu and loss function is continu
        loss = abs_err_gt*weighted_err + (1-abs_err_gt)*weighted_sqerr
